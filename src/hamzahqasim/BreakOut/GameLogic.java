...
public class GameLogic extends JPanel implements KeyListener, ActionListener {
    ...
    private int ballXdir = 1; // Ball X direction step
    private int ballYdir = 2; // Ball Y direction step
    ...
    public void paint(Graphics g){
        ...
        // Game Lose Scenario
        if (ballYpos > GameApp.gameWindowHeight - 23){
            playStatus = false;
            ballYdir = 0;
            ballXdir = 0;
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        ...
        if(playStatus) {
            ...
            if (new Rectangle(ballXpos,ballYpos,20,20).intersects(new
                ...
                ballYdir = -ballYdir;
                // Rebound Mechanics
                if (ballXpos <= (paddleX + (paddleWidth / 2) ) ) {
                    if (ballXPreImpact < ballXpos){
                        ballXdir = -ballXdir;
                    }
                } else if (ballXpos > (paddleX + (paddleWidth / 2) ) ){
                    if (ballXPreImpact > ballXpos){
                        ballXdir = -ballXdir;
                    }
                }
